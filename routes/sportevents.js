	    ...
	    if(searchToken == "BESTNEARBY")
{
	     ...
	     dist = 100.0;
	     Event.find({eventDate : { $gt: nowDate, $lt: nextDate }}).populate('placeId', null, {loc: {$maxDistance: dist/111.2, $near: [lat,lon]} }).populate('hometeamId').populate('awayteamId').populate('eventTypeId').exec(function(err, pl) { 
	    res.send({events: pl});
	    });
	    }else{
	    ...
	    dist = 100.0;
	    Event.find({eventDate : { $gt: nowDate, $lt: nextDate }}).populate('placeId', null, {loc: {$maxDistance: dist/111.2, $near: [lat,lon]} }).populate('eventTypeId',null,{'name' : searchToken}).populate('hometeamId').populate('awayteamId').exec(function(err, pl) { 
	    res.send({events: pl});
	    });
...
//User manually input the text or click on an item
	     ...
	     Event.find({eventDate : { $gt: nowDate, $lt: nextDate }, eventDescription:{$regex: new RegExp(searchString, 'i') }}).populate('placeId').populate('hometeamId').populate('awayteamId').populate('eventTypeId').exec(function(err, pl) { 
	    res.send({events: pl});
	  });
